...
<div id="canvasContainer1">
	<canvas id="canvasSerialRandom" height = "300" style="float:left; width:30%;"></canvas>
	<canvas id="canvasSerialGrid"   height = "300" style="float:left; width:30%;"></canvas>
</div>
...
<div id="buttonContainer1" height = "300" style="width:60%; display:flex; align-items:center;">
	<button id="buttonNext"  type="button" style="height:40px; width:120px; margin-left:auto; margin-right:10px;">Next<br>hyperparameter</button>
	<button id="buttonClear" type="button" style="height:40px; width:120px; margin-left:10px; margin-right:auto;">Clear</button>
</div>

<script>
	const ctxRandom = document.getElementById('canvasSerialRandom').getContext('2d');
	const ctxGrid   = document.getElementById('canvasSerialGrid').getContext('2d');
	
	var chartRandom = createSerialChart(ctxRandom, false);
	var chartGrid = createSerialChart(ctxGrid, true);
...
In the stochastic setting each time one pulls a slot machine a reward is sampled from a fixed distribution with mean $$ \mu $${% sidenote 'sn-id-5' "We'll use $$ \mu $$ to denote the mean of the distribution in the stochastic setting. For the non-stochastic setting, we'll go with the authors and use $$ \nu $$ to denote the limiting loss."%} i.e. 
...
{% marginnote 'tool-id-3' "The configuration with the lowest limiting loss is what we're after!" %}
...
<div id="buttonContainer2" height="300" style="width:50%; display:flex; align-items:center;">
	<button id="buttonAdd"    type="button" style="height:40px; width:120px; margin-left:auto; margin-right:10px;">Add<br>hyperparameter</button>
	<button id="buttonRemove" type="button" style="height:40px; width:120px; margin-left:10px; margin-right:auto;">Remove<br>hyperparameter</button>
</div>
...
The authors take advantage of the following key insight: promising configurations tend to perform great even early on in the process. Therefore there is no need to keep running the bad-performing configurations; they probably suck anyway{% sidenote 'sn-id-6' "Fine print: this is merely a heuristic for prunning the bad-performing configurations. It's not a silver bullet! Check out the sufficiency result."%}.

### Basic outline

Here we see a very basic, barebones version of the algorithm without any distracting equation or variable. By taking advantage of the previous insight, the algorithm gradually prunes{% sidenote 'sn-id-7' "This pruning action is also known as *early-stopping*."%} some of the bad-performing arms. 
...
- The number of arms ($$ n $$) needs to be specified explicitly (therefore needs to be finite){% sidenote 'sn-id-8' "This is a very big limitation. What if the hyperparameter space $$ \mathcal{H} $$ is continuous (which is often the case)?"%}. 
